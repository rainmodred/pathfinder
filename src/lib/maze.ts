...
import { getRandomEvenNumber, getRandomOddNumber } from './utils.ts';
...
export function createMaze(grid: Grid, nodesToAnimate: Node[]) {
	...
	recursiveDivision(2, 2, grid.cols - 3, grid.rows - 3, 'VERTICAL', nodesToAnimate);
}
...
export function recursiveDivision(
	...
	if (orientation === 'VERTICAL') {
		...
		if (height - col > wallX - 2 - row) {
			recursiveDivision(row, col, wallX - 2, height, 'HORIZONTAL', nodesToAnimate);
		} else {
			recursiveDivision(row, col, wallX - 2, height, orientation, nodesToAnimate);
		}

		if (height - col > width - (wallX + 2)) {
			recursiveDivision(wallX + 2, col, width, height, 'HORIZONTAL', nodesToAnimate);
		} else {
			recursiveDivision(wallX + 2, col, width, height, orientation, nodesToAnimate);
		}
	} else {
		...
		if (wallY - 2 - col > width - row) {
			recursiveDivision(row, col, width, wallY - 2, orientation, nodesToAnimate);
		} else {
			recursiveDivision(row, col, width, wallY - 2, 'VERTICAL', nodesToAnimate);
		}
		if (height - (wallY + 2) > width - row) {
			recursiveDivision(row, wallY + 2, width, height, orientation, nodesToAnimate);
		} else {
			recursiveDivision(row, wallY + 2, width, height, 'VERTICAL', nodesToAnimate);
		}
	}
}
